...
test('happy path: discover -> resolve(NotPaid) -> wire -> settled', async () => {
  ...
  const row = (await pool.query("SELECT id, status, destination->>'bankAccountNumber' acc, array_length(service_ids,1) n FROM zampay_settlements WHERE transaction_id=$1", [txn])).rows[0];
  assert.equal(row.status, 'READY_TO_WIRE');
  assert.equal(row.acc, '0132030000194');
  assert.equal(row.n, 2);

  await orch.confirmWired(row.id, 'WIRE-REF-9', randomUUID());
  assert.equal((await pool.query('SELECT status FROM zampay_settlements WHERE id=$1', [row.id])).rows[0].status, 'WIRED');
  ...
  assert.equal(sink[0].paymentReferenceNumber, 'WIRE-REF-9');
...
test('two destinations on one invoice -> two instructions', async () => {
  ...
  assert.ok(rows.every((r) => r.status === 'READY_TO_WIRE'));
});

test('Paid invoice -> INVOICE_PAID, never wired', async () => {
...
test('callback failure increments attempts and stays pending', async () => {
  ...
  await orch.confirmWired(id, 'WIRE-X', randomUUID());
  ...
  assert.equal(after1.status, 'WIRED');
