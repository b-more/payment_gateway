...
//
// Exercises the public v1 API end-to-end (signed requests) and asserts each
// step, including negative security cases. No dependencies — Node 18+ only.
//
// USAGE:
//   IC_API_KEY=ic_sand_xxx \
//   IC_SIGNING_KEY=your_signing_key \
...
//   IC_PROCESSOR  (default MTN)            MTN | AIRTEL
//   IC_AMOUNT     (default 5000)           integer ngwee (5000 = K50.00)
//
// Get IC_API_KEY / IC_SIGNING_KEY from the Merchant Portal → API Documentation
// (regenerate a SANDBOX key to reveal the signing key). Use a SANDBOX key: it
// simulates, never moves real money, and never touches production float.
// ─────────────────────────────────────────────────────────────────────────────
...
if (!API_KEY || !SIGNING_KEY || !ACCOUNT_ID) {
  console.error(`${C.red}Missing config.${C.reset} Set IC_API_KEY, IC_SIGNING_KEY and IC_ACCOUNT_ID.`);
  console.error('Example:\n  IC_API_KEY=ic_sand_… IC_SIGNING_KEY=… IC_ACCOUNT_ID=… node testing/api-smoke-test.mjs');
  process.exit(2);
}
...
async function call(method, path, bodyObj, opts = {}) {
  ...
  const headers = { 'X-Api-Key': API_KEY, 'X-Timestamp': ts, 'X-Signature': signature };
  if (method !== 'GET') {
...
await test('SEC  tampered signature            → 401 rejected', async () => {
  const r = await call('POST', '/v1/collections', { processor: PROCESSOR, amount: AMOUNT, msisdn: MSISDN }, { badSig: true });
  expect(r.status === 401, `expected 401, got ${r.status}`);
});

await test('SEC  stale timestamp (−1h)         → 401 rejected', async () => {
  const r = await call('POST', '/v1/collections', { processor: PROCESSOR, amount: AMOUNT, msisdn: MSISDN }, { staleTs: true });
  expect(r.status === 401, `expected 401, got ${r.status}`);
});
