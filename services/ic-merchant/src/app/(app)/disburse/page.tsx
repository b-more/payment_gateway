...
function failureText(reason: string | null | undefined): string {
  if (reason === 'INSUFFICIENT_FLOAT') return 'Insufficient float to cover this payout — fund the account first.';
  if (reason === 'SIMULATED_DECLINE') return 'Declined (sandbox simulation).';
  return reason ? `This payout did not complete (${reason}).` : 'This payout did not complete.';
}
...
export default function DisbursePage(): ReactNode {
  ...
  async function submit(e: React.FormEvent): Promise<void> {
    ...
    setResult(null);
    try {
      const r = await apiPost<TxnResult>(`/v1/merchant/accounts/${accountId}/disburse`, {
        processor,
        amount: ngwee,
        msisdn: msisdn.trim(),
      ...
      setError(err instanceof ApiError ? friendlyError(err, processor) : 'Could not start the payout.');
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <>
      <PageHead title="Disburse" subtitle="Send a payout to a customer's mobile money." />

      {accounts.loading ? (
        <Spinner />
      ) : list.length === 0 ? (
        <Empty>No accounts yet. Once your account is provisioned, you can send payouts here.</Empty>
      ) : (
          ...
          <div className="card card-pad">
            ...
            <form onSubmit={(e) => void submit(e)}>
              ...
              <button className="btn primary" disabled={!canSubmit}>
                {busy ? 'Sending…' : ngwee ? `Send ZMW ${amount}` : 'Send payout'}
              </button>
            </form>
          </div>
            ...
            {!result ? (
              <div className="result-state">
                ...
                <div className="result-sub" style={{ maxWidth: 250, margin: '0 auto' }}>
                  Enter the recipient and amount, then <b>Send</b>. The money is paid straight to their mobile
                  wallet. The result appears here and under <b>Transactions</b>.
                </div>
              </div>
            ) : result.status === 'SUCCESS' ? (
              <div className="result-state">
                <div className="result-icon ok">{ICON_CHECK}</div>
                <div className="result-title">Payout sent</div>
                <div className="result-amount">{zmw(result.amount)}</div>
                <div className="result-sub">to {result.msisdn ?? '—'} · {result.processor.replace('_', ' ')}</div>
                ...
                <button className="btn primary" onClick={newPayout}>Send another</button>
              </div>
            ) : result.status === 'PROCESSING' ? (
              <div className="result-state">
                <div className="result-icon pending">{ICON_SEND}</div>
                <div className="result-title">Sending payout</div>
                <div className="result-amount">{zmw(result.amount)}</div>
                <div className="result-sub">to {result.msisdn ?? '—'}</div>
