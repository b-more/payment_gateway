...
export default function ReportsPage(): ReactNode {
  const { data, loading, error, reload } = useData<Report[]>('/v1/merchant/reports');
  ...
  async function download(r: Report, format: 'pdf' | 'csv'): Promise<void> {
    ...
    try {
      const path = format === 'pdf' ? `/v1/merchant/reports/${r.id}/pdf` : `/v1/merchant/reports/${r.id}/export`;
  ...
  return (
    <>
      <PageHead
        title="Reports"
        subtitle="Generate your transactions and settlements and export them as branded PDF or CSV."
        actions={
...
function CreateReport({ onDone, onError }: { onDone: () => void; onError: (m: string) => void }): ReactNode {
  ...
  async function submit(e: FormEvent): Promise<void> {
    ...
    setBusy(true);
    try {
      await apiPost('/v1/merchant/reports', { name: f.name, reportType: f.reportType, from: f.from, to: f.to });
      onDone();
    } catch (err) {
    ...
    <div className="card card-pad" style={{ marginBottom: 16 }}>
      ...
      <form onSubmit={(e) => void submit(e)}>
        ...
        <div className="row" style={{ margin: '0 0 16px' }}>
          <span className="muted" style={{ fontSize: 12 }}>Quick range:</span>
